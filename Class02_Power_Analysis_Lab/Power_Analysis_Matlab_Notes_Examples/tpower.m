...
% Adapted from Jason Augustyn (2025)
%   X=TPOWER(N,D,ALPHA) returns the power of a t-test at ALPHA with (N-1)
%   degrees of freedom, for effect size d. For a two-sample test N is the sample size per condition, NOT the total
...
if nargin >= 5
    nreq_idx = max(find(((despwr-.01)<dpower)&(dpower<(despwr+.01))));
    nreq=N(nreq_idx);
    if plot == 1
        hold on;
